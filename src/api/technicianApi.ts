...
export const technicianApi = {
  async login(input: LoginInput) {
    const response = await fetch(`${environment.apiBaseUrl}/api/v1/auth/login/technician`, {
      method: 'POST',
  ...
  async logout() {
    ...
    if (refreshToken) {
      try {
        await requestData<null>({method: 'POST', url: '/api/v1/auth/logout', data: {refreshToken}});
      } catch {
  ...
  dashboard() {
    return requestData<TechnicianDashboard>({method: 'GET', url: '/api/v1/technician/me/dashboard'});
  },

  profile() {
    return requestData<TechnicianProfileView>({method: 'GET', url: '/api/v1/technician/me/profile'});
  },
  ...
  jobs(query: JobsQuery = {}) {
    return requestData<PageView<RequestView>>({
      ...
      params: {page: query.page ?? 0, size: query.size ?? 20, status: query.status},
    });
  },

  job(id: number) {
    return requestData<JobDetail>({method: 'GET', url: `/api/v1/technician/me/jobs/${id}`});
  },
  ...
  updateLocation(id: number, input: {latitude: number; longitude: number; timestamp: string}) {
  ...
  visit(id: number) {
    return requestData<VisitView>({method: 'GET', url: `/api/v1/technician/me/visits/${id}`});
  },

  updateVisitStatus(id: number, status: Extract<VisitStatus, 'IN_PROGRESS' | 'COMPLETED'>, notes?: string) {
  ...
  attachments(requestId: number) {
    return requestData<AttachmentView[]>({method: 'GET', url: `/api/v1/service-requests/${requestId}/attachments`});
  },
  ...
  markNotificationRead(id: number) {
    return requestData<NotificationView>({method: 'PUT', url: `/api/v1/notifications/${id}/read`});
  },
};
